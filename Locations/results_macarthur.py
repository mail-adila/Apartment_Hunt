...
class ResultsMacArthur(object):

    def __init__(self, driver):
        self.driver = driver
        self.apartment_links = driver.find_elements(By.XPATH, Locator.ma_apartment_links)
    ...
    def get_listings(self):
        ...
        try:
            for apartment in apartments_on_this_floor:
                ...
                beds = ((self.driver.find_element(By.XPATH, Locator.ma_text_beds).text.split('|')[0]).split(' ')[0])
                floor = (self.driver.find_element(By.XPATH, Locator.ma_text_floors).text.split('#')[1])
                floor = floor[2:3]
                if int(beds) > 1 and int(floor) < 3:
                    garage_status = False
                    garage_status_2 = False
                    try:
                        garage_status = self.driver.find_element(By.XPATH, Locator.ma_text_garage_status)
                    except NoSuchElementException:
                        garage_found = False
                    try:
                        garage_status_2 = self.driver.find_element(By.XPATH, Locator.ma_text_garage_status_2)
                    except NoSuchElementException:
                        garage_found = False
                    garage_found = garage_status or garage_status_2
                    try:
                        availability = self.driver.find_element(By.XPATH, Locator.ma_text_availability).text
